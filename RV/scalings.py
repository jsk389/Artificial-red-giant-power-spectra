...
class Scalings(object):
    ...
    def d02_scaling(self):
        ...
        d02 = 0.112 +/- 0.016
        """
        return 0.112 * self.dnu #np.random.normal(0.121, 0.003, self.n_env) * self.dnu + \
#               np.random.normal(0.035, 0.012)
    ...
    def d01_scaling(self):
        ...
        """
        return self.dnu/2.0 + 0.109 #np.random.normal(0.109, 0.012, self.n_env)
...
#               np.random.normal(0.632, 0.032) * np.log10(self.dnu)
    ...
    def amax_scaling(self):
        ...
        if self.mission == 'Kepler':
            ...
            print("SONG!")
            # Need tp scale H_env properly for velocity from intensity!
            self.H_env = self.H_env * (1.0 / 400.0) * (self.Teff / 5777.)
            self.amax = np.sqrt(self.H_env * self.dnu / self.vis_tot)  
